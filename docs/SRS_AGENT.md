...
| REQ-AGENT-062 | NON-GOAL. Only free-standing content blocks (see `isContentBlock`, diagram-owned text excluded) shall reflow on insert/move. Diagram frames, their members, shapes, images and strokes shall hold their y — an inserted block that lands where a diagram sits overlaps it, exactly as an appended block would if y collided. Frame-resize reflow (blocks below a diagram moving when the frame is redrawn) remains descoped per 2026-08-13 (PLAN v0.34.0, SRS_DIAGRAM "Not built", REQ-DIAG-002..005) | Must | T161 |
